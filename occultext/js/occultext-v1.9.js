...
document.getElementById("decrypt-form").addEventListener("submit", function (event) {
    ...
    showLoading();
    fetch(
        `https://frenzvalios.pythonanywhere.com/api/occultext/decrypt/atlas?occultext=${encodeURIComponent(
            occultext
...
function removeSpaces() {
    ...
    occultextTextarea.value = occultextTextarea.value.trimEnd();
}
function removeAllSpaces() {
    const occultextTextarea = document.getElementById("occultext");
    let occultext = occultextTextarea.value.trimEnd();
    if (occultext.includes("{") && occultext.includes("}")) {
        occultext = occultext.replace(/\s+(?=(?:[^{}]*\{[^{}]*\})*[^{}]*$)/g, (match) =>
            match.trim() === "" ? "" : " "
        );
    }
